var Administrador = function(){
    ...
    this.table = "<% _.each(as, function(ad){ %> <tr><td><%= ad.Correo %></td><td><%= ad.Nivel==0?'Super Administrador':'Administrador' %></td><td><%= ad.UltimaSesion == 'NULL' ? 'No ha iniciado sesión' : tools.printDate(ad.UltimaSesion) %></td><td><span class='label <%= ad.Estado==1?\"label-success'>Activo\":\"label-warning'>Inactivo\" %></span></td></tr> <% }) %>";
